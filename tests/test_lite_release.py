...
class LiteReleaseTests(unittest.TestCase):
    def test_version_is_frozen(self):
        self.assertEqual((SKILL / "VERSION").read_bytes(), b"0.1.0\n")
    ...
    def test_release_facing_readmes_have_no_dev_identity(self):
        for relative in (
            ...
            self.assertIn("0.1.0", text, relative)
            self.assertNotIn("0.1.0-dev", text, relative)
    ...
    def test_public_commands_are_frozen(self):
        ...
        self.assertEqual(set(subparsers.choices), {"init", "doctor", "check-start", "check-result"})
    ...
    def test_release_notes_cover_release_contract(self):
        ...
        for required in (
            "KyMCM Lite 0.1.0", "Python 3.11", "3.12", "3.13", "init", "doctor",
            "check-start", "check-result", '{"workflow":"kymcm_lite","version":3}',
