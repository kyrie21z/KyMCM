...
在指定 workspace 下维护独立的 Git 仓库，与 KyMCM Skill 源目录完全分离。
...
def configure_workspace(value: str | Path | None = None) -> Path:
    global WORKSPACE, GIT_ROOT, GIT_DIR
...
# ── 公开 API ──────────────────────────────────────────────────────────────
def init(contest_name: str = "", author: str = "KyMCM Agent") -> None:
    """
    在 CUMCM_Workspace/ 下初始化 Git 仓库。
    工作区必须已由 setup_workspace.py 创建。
    """
    if not WORKSPACE.exists():
        print(f"[contest_git] ERROR: workspace 不存在: {WORKSPACE}",
              file=sys.stderr)
        sys.exit(1)
...
def auto_commit(stage: str, round_n: int = 1) -> str | None:
    """
    ...
    # 构造语义化提交消息
    msg = f"chore({stage}): workflow snapshot {round_n}"
...
def main():
    p = argparse.ArgumentParser(
        description="KyMCM 竞赛工作区 Git 版本控制",
    ...
    pi.add_argument("--author", default="KyMCM Agent", help="git user.name")
