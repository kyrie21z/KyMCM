...
## P3.1 - Figure core closure

- Stabilized semantic Proposed/Baseline/Alternative styles with explicit role overrides and label-hash fallback.
- Added Chinese and English built-in template text.
- Added output hashes, sizes, temporary rendering, basic SVG/PDF/PNG validation, and strict preferred-PDF audit.
- Restricted each Figure Brief to exactly one traceable CSV evidence source.

## P3.0 - Figure system

- Added stateless Figure Briefs, bounded chart selection, and a shared paper-oriented theme.
- Added six data-driven templates with SVG/PDF/PNG export and source-hashed manifests.
- Added deterministic Figure Audit, a generated HTML gallery, and a writing-module manifest interface.
- Recorded third-party provenance and used an independent mechanism rewrite without copying code, assets, or fonts.

## P2.2 - Final Contract closure and freeze

- Made pending human approval mutually exclusive with active problem execution.
- Rechecked Contract front matter identity at approval and during approved-state validation.
- Required active and completed problems to retain current approved Problem and Solution Contracts.
- Froze the Contract architecture after schema v4 lifecycle closure.

## P2.1 - Contract lifecycle closure

- Removed `run-stage` and per-problem `stages` from the active schema v4 workflow.
- Allowed user selection among independent ready problems while enforcing one active focus.
- Bound Solution Contracts, problem validation, global validation, and paper approval to versioned SHA-256 evidence snapshots.
- Added conservative global invalidation for new Problem Contract versions and DAG-region invalidation for new Solution Contract versions.
...
- New script `scripts/contest_git.py` — manages an independent Git repo inside
  ...
  - Read-only queries: `log(n, oneline)`, `diff(ref1, ref2, stat_only)`,
    `status()`, `list_tags()`
  - Standalone CLI: `python "$KYMCM_ROOT/scripts/contest_git.py" --workspace "$WORKSPACE" init|log|diff|status|tag|tags`
...
- `auto-mcm/SKILL.md`: **【建模质量门控】** updated — now documents exact
  `python "$KYMCM_ROOT/scripts/quality_gate.py" --workspace "$WORKSPACE"` invocations for every pipeline moment
