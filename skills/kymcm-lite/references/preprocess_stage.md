...
每项 EDA 必须直接服务数据理解、模型设计或风险识别，并有明确目的、结构化统计或必要诊断资产、分组口径、证据和可支持/不可支持推断。区分描述性事实、探索性发现和待验证假设。相关、组间差异和时间共变不证明因果；样本外、时间外、空间外或未覆盖群体不得无依据外推。预测任务在任何全样本变换、时间切分和目标衍生前检查泄漏。不因展示需求扩展 EDA，也不决定最终图形。
