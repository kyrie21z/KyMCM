...
For modeling, treat `FROZEN_CONTEXT.md`, the current `START_QN.md`, the current `RESULT_QN.md`, and RESULT-declared evidence as the formal Lite surfaces. Read the complete context and START before implementation, and the complete RESULT plus evidence before external review. Use `scripts/lite.py` for initialization and read-only checks, and follow `references/markdown_format.md`.
