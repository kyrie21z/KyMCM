...
KyMCM Lite 0.8.1 keeps the separate Lite v3 marker `{"workflow":"kymcm_lite","version":3}`, every START/RESULT/HANDOFF/APPENDIX heading, START/RESULT single/split identities, dependencies, evidence rules, figure/appendix behavior, and eight public commands from 0.8.0. Existing modeling workspaces remain valid.
...
RESULT remains formal, concise, auditable, and the only downstream modeling inheritance surface. HANDOFF is a derived neutral technical-transfer document under `problems/qN/notes/`, never a formal fact source or modeling dependency. Single and split modes both use one problem-level `HANDOFF_QN.md`; split mode creates it only after all contiguous RESULT units pass, and it preserves their exact identities and certification boundaries. Appendix organization may reference but cannot copy HANDOFF.
