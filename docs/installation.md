...
Use Python 3.11, 3.12, or 3.13. Copy either KyMCM Full 1.0.0 (`skills/kymcm-full`) or KyMCM Lite 0.3.1 (`skills/kymcm-lite`) into the Codex skills directory used by your installation; each copied Skill is self-contained.
...
Users who installed Lite by copying the directory must replace/reinstall the complete Skill after updating to 0.3.1. Symlink installations need only update the repository and restart Codex.

To migrate a 0.2.0 Lite workspace, add exactly one `**前问依赖：** ...` line to section 2 of every START, ensure every declared predecessor has a completed START and RESULT, and rerun `doctor`, `check-start`, and `check-result`. A legacy `FROZEN_CONTEXT.md` may be deleted manually or left in place; 0.3.1 ignores it completely. Existing 0.3.0 single-mode workspaces require no rewrite.
