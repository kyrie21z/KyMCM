...
Use Python 3.11, 3.12, or 3.13. Copy either KyMCM Full 1.0.0 (`skills/kymcm-full`) or KyMCM Lite 0.4.0 (`skills/kymcm-lite`) into the Codex skills directory used by your installation; each copied Skill is self-contained.
...
Users who installed Lite by copying the directory must replace/reinstall the complete Skill after updating to 0.4.0 so the bundled `references/modeling_plan_design.md` is present. Symlink installations need only update the repository and restart Codex.

Existing 0.3.1 single/split workspaces require no structural rewrite for 0.4.0. Apply the modeling-plan standard when a START is next materially revised, reviewed, or executed; revise sections 5–7 before formal execution when smoke testing, recovery boundaries, or realistic cost design are missing. Do not rewrite completed historical contracts mechanically.
